...
detetion_paths = {
    ...
    'tless': '../Instance_Segmentation_Model/log/sam/result_tless_gt_scene_01-10.json',
...
def get_parser():
    ...
    parser.add_argument("--icp_mode",
                        type=str,
                        choices=["none", "point2point", "point2plane"],
                        default="none",
                        help="Choose ICP refinement mode.")
...
def run_icp_refinement(source_pts, target_pts, target_normals=None, threshold=0.01, mode="point2plane"):
    """
    Runs ICP (point-to-plane or point-to-point)

    Args:
        source_pts: Nx3 np.ndarray
        target_pts: Nx3 np.ndarray
        target_normals: Nx3 np.ndarray or None
        threshold: float (meters)
        mode: "point2plane" or "point2point"

    Returns:
        result_icp: ICP result (transformation, fitness, rmse)
    """
            ...
            for label, proposals_for_obj in cluster_proposals.items():
                ...
                for i in range(0, N, BATCH_LIMIT):
                    ...
                    for proposal in batch_proposals:
                        ...
                        # save_dir = f"/home/obuset/SAM-6D/SAM-6D/Pose_Estimation_Model/log/intermediate_results_DBSCAN_ICP_P2Plane_DB_ICP_P2Plane_corrected7/intermediate_results_scene_{proposal['scene_id'].item():02d}_v_DB_ICP_P2Plane_corrected7/"
                ...
                if cfg.icp_mode == "point2plane" or cfg.icp_mode == "point2point":
                    print("[ICP] Running ICP refinement...")
                   
                    for idx, proposal in enumerate(used_proposals):
                        obj_id = proposal['obj_id'].item()
        
                        # Skip if this obj_id was already tested (optional but avoids redundancy)
                        # if obj_id in [r[1] for r in icp_results]:
                        #     continue

                        R_pred = end_points['pred_R'][idx].cuda()
                        t_pred = end_points['pred_t'][idx].view(3, 1).cuda()

                        # Transform to world frame
                        R_c2w = proposal['cam_R_w2c'].squeeze(0).T
                        t_c2w = -R_c2w @ (proposal['cam_t_w2c'].squeeze(0) / 1000.0).view(3, 1)
                        R_final = R_c2w @ R_pred
                        t_final = (R_c2w @ t_pred + t_c2w).view(-1)

                        # Load model point cloud
                        cad_path = f"{BASE_DIR}/../Data/BOP/tless/models_cad/obj_{obj_id:06d}.ply"
                        mesh = trimesh.load_mesh(cad_path)
                        model_pts = mesh.sample(512).astype(np.float32)
                        model_pts /= 1000.0  # mm → m
                        
                        # Transform model
                        transformed_model_pts = (R_final.cpu() @ model_pts.T + t_final.view(3, 1).cpu()).T.numpy()

                        # ICP
                        icp_result = run_icp_refinement(transformed_model_pts, merged_pts.cpu().numpy(), mode=cfg.icp_mode)
                        fitness = icp_result.fitness

                        # Combine scores
                        pem_score = end_points['pred_pose_score'][idx].item()
                        ism_score = proposal['score'].item()
                        combined_score = ism_score * fitness
                    ...
                    # fitness, best_obj_id, best_R_final, best_t_final = max(icp_results, key=lambda x: x[0])

                    best_result = max(icp_results, key=lambda x: x['score'])
                    fitness = best_result['fitness']
                    best_obj_id = best_result['obj_id']
                    best_R_final = best_result['R']
                    best_t_final = best_result['t']
                    print(f"[ICP] Chose obj_id={best_obj_id} with fitness={fitness:.4f}")
                    
                    # Select best based on combined score
                    best_result = max(icp_results, key=lambda x: x['score'])

                    # Define what you used before
                    best_proposal = best_result['proposal']
                    best_proposal_obj_id = best_result['obj_id']

                    # Save
                    cluster_results[label] = {
                        'obj_id': best_result['obj_id'],
                        'R': best_result['R'],
                        't': best_result['t'],
                        'pem_score': best_result['score'],  # Now includes ICP + PEM + ISM
                        'merged_points': merged_pts
                    }

                    print(f"\n[Cluster {label}] Best Proposal: obj_id: {best_proposal_obj_id}, score: {best_result['score']:.4f}")

                    # Rebuild a map from proposal to combined score (or just print from icp_results)
                    for idx, proposal in enumerate(used_proposals):
                        obj_id = proposal['obj_id'].item()
                        # Get the corresponding score from icp_results
                        # score_entry = next((res for res in icp_results if res['proposal'] == proposal), None)
                        score_entry = next((res for res in icp_results if res['proposal'] is proposal), None)

                        if score_entry is not None:
                            print(f"Proposal {idx}: obj_id: {obj_id}, score: {score_entry['score']:.4f}")
                        else:
                            print(f"Proposal {idx}: obj_id: {obj_id}, score: N/A")

                else:
                    print("[ICP] No ICP refinement applied.")
                        
                    # Find the best proposal based on the highest score
                    pred_scores = end_points['pred_pose_score'] * end_points['score']
                    if len(pred_scores) == 0:
                        print(f"[Cluster {label}] No valid proposals found, skipping.")
                        continue
                    # continue
            ...
            for view_proposals in sorted_views:
                ...
                for label, result in cluster_results.items():
                    ...
                    if cfg.visualization:
                        ...
                        vis_dir = save_path.replace(f'result_tless_scene_01-10_v_11_corr_ICP_P2P_GT.csv',f"pem_visualization/scene_{scene_id:02d}_v_11_corr_ICP_P2P_GT/")
...
if __name__ == "__main__":
    ...
    if cfg.dataset == 'all':
        ...
        if not os.path.isdir(save_path):
            os.makedirs(save_path)
        save_path = os.path.join(save_path,'result_' + dataset_name + f'_scene_01-10_v_11_corr_ICP_P2P_GT'+'.csv')
        test(model, cfg,  save_path, dataset_name, detetion_paths[dataset_name])
